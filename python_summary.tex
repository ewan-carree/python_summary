...
Here is an iterable object that is also an iterator :
\begin{lstlisting}[language=Python]
class MyRange:
...
We can also create the equivalent of MyRange class with a function based on \hyperref[sec:Generators]{generator} which is more readable than a class :
\begin{lstlisting}[language=Python]
...
Each instance can have its own attributes, which defines its state. An instance can also use methods from the class to modify its state.
\begin{lstlisting}[language=Python]
...
\subsection{Methods}
Methods are functions that bellow to the class. Tey are basically functions that uses attributes from the class. A method can be used by the instance as follow  : \textbf{my\_instance.my\_method()} except for class methods where \textbf{My\_class.my\_method()} is better. \\
...
Dunder methods are specific function from a class, they are surrounded by \_\_. A dunder method can be used by the instance as follow :\\ \textbf{my\_instance.\_\_dunder\_method\_\_} but it's better to use them like that : \textbf{dunder\_method(my\_instance)}
\begin{lstlisting}[language=Python]
class Point(object):
...
def people_generator(num_people):
    for i in xrange(num_people):
        ...
        yield person
\end{lstlisting} \vspace{5mm}
Here is an example of list against generator. If you run these function with lot of people, you'll see the powerness of generators, instead of returning a list of lot of people, it will yield one peron a the time. Also, it won't take any memory or any time instead of a list. \\
...
You can also you generator as comprehension like with list to save memory and time simply by replacing the \textbf{[]} from list comprehension with \textbf{()} for generator comprehension. \\
