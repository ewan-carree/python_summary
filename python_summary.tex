...
\subsection{Logging}
Logging is a way to capture information into a specific file with a .log extension. It's useful when we need to capture info while the program is running.

\vspace{5mm}

There are 5 levels of logging :

First, there are the non default levels :
\begin{itemize}
    \item DEBUG
...
You can change your level to non default level by using : 
\begin{lstlisting}[language=Python]
logging.basicConfig(filename='my_file.log', level=logging.DEBUG) # or level=logging.INFO
\end{lstlisting} \vspace{5mm}

Then, there are the default level :
\begin{itemize}
    \item WARNING
...
You can change the way to write information by adding an argument to the \textit{basicConfig()} methods which is \textit{format='\%(asctime)s:\%(levelname)s:\%(message)s'}, you can find other way to format information \href{https://docs.python.org/3/library/logging.html#logrecord-attributes}{here}

\vspace{5mm}

To write into your file, simply replace \textit{print()} by \textit{logging.info/debug/warning/error/critical()}.
